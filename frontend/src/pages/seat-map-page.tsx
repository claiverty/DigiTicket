...
export function SeatMapPage() {
  ...
  if (eventQuery.isPending || seatsQuery.isPending) return <div className="mx-auto max-w-6xl px-6 py-20 text-slate-400">Carregando mapa…</div>;
  if (eventQuery.isError || seatsQuery.isError) return <div className="mx-auto max-w-6xl px-6 py-20 text-rose-200">Evento não encontrado ou o mapa não pode ser alterado.</div>;
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-14">
      ...
      <h1 className="mt-5 text-4xl font-semibold text-white">Mapa de {eventQuery.data.title}</h1>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_22rem]">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5 sm:p-7">
          ...
          {seats.length === 0 ? (
            <p className="py-16 text-center text-slate-400">Adicione o primeiro setor para gerar o mapa.</p>
          ) : (
            <div className="overflow-x-auto">
              <div className="mx-auto w-max space-y-8">
                {sections.map((section) => (
                  <div key={section.id}>
                    <p className="mb-3 text-center text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{section.name} · {formatMoney(section.priceCents)}</p>
                    <div className={section.seatDisplaySize === 'LARGE' ? 'space-y-3' : 'space-y-2'}>
                      {[...(sectionRows.get(section.id) ?? new Map<string, typeof seats>()).entries()].map(([rowLabel, rowSeats]) => (
                        <div key={rowLabel} className={section.seatDisplaySize === 'LARGE' ? 'flex items-center gap-3' : 'flex items-center gap-2'}>
                          <span className="w-6 text-center text-xs text-slate-500">{rowLabel}</span>
                          {rowSeats.map((seat) => <span key={seat.id} title={seat.ticketType.name} className={`flex items-center justify-center rounded-lg bg-emerald-400/20 text-xs text-emerald-200 ${section.seatDisplaySize === 'LARGE' ? 'h-12 w-12' : 'h-9 w-9'}`}>{seat.seatNumber}</span>)}
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          {sections.length > 0 && (
            <div className="mt-8 border-t border-white/10 pt-5">
              <h2 className="text-sm font-semibold text-white">Setores configurados</h2>
              <div className="mt-3 space-y-3">
                {sections.map((section) => (
                  <div key={section.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 p-3 text-sm">
                    <span className="font-medium text-slate-200">{section.name} · {formatMoney(section.priceCents)}</span>
                    <div className="flex flex-wrap gap-2">
                      <button type="button" disabled={sizeMutation.isPending} onClick={() => sizeMutation.mutate({ ticketTypeId: section.id, size: section.seatDisplaySize === 'LARGE' ? 'STANDARD' : 'LARGE' })} className="rounded-lg border border-emerald-300/20 px-3 py-1.5 text-emerald-200">
                        {section.seatDisplaySize === 'LARGE' ? 'Usar tamanho padrão' : 'Usar tamanho VIP'}
                      </button>
                      <button type="button" disabled={deleteMutation.isPending} onClick={() => window.confirm(`Excluir o setor “${section.name}” e seus assentos?`) && deleteMutation.mutate(section.id)} className="rounded-lg border border-rose-300/20 px-3 py-1.5 text-rose-200">Excluir</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <form onSubmit={(event) => { event.preventDefault(); saveMutation.mutate(); }} className="h-fit space-y-5 rounded-2xl border border-white/10 bg-white/5 p-6">
          <h2 className="text-xl font-semibold text-white">Novo setor</h2>
          ...
          <label>
            <span className="mb-2 block text-sm font-medium text-slate-200">Tamanho visual</span>
            <select value={seatDisplaySize} onChange={(event) => setSeatDisplaySize(event.target.value as 'STANDARD' | 'LARGE')} className="w-full rounded-xl border border-white/10 bg-slate-900 px-4 py-3 text-white outline-none focus:border-emerald-400">
          ...
          {mutationError && <p role="alert" className="rounded-xl bg-rose-400/10 p-3 text-sm text-rose-200">{mutationError instanceof ApiError ? mutationError.message : 'Não foi possível alterar o mapa.'}</p>}
          <button type="submit" disabled={saveMutation.isPending} className="w-full rounded-xl bg-emerald-400 px-4 py-3 font-semibold text-slate-950 disabled:opacity-50">{saveMutation.isPending ? 'Gerando…' : 'Adicionar setor'}</button>
        </form>
      </div>
    </section>
  );
